...
if nargin<9
    ...
    [FilenameVideo, PathVideo]  = uigetfile('*.*','Select video file','e:\Projects\LNOF\BehaviorData\2_Combined\');
    [FilenameDLC, PathDLC]  = uigetfile('*.csv','Select DLC file with body parts','e:\Projects\LNOF\BehaviorData\3_DLC\');
    PathOut = uigetdir('e:\Projects\LNOF\BehaviorData\5_Behavior\', 'Pick a Directory for Outputs');
    
    % loading preset file
    answer = questdlg('Do you have preset file?', 'Uploading files', 'Yes','No','Yes');
    switch answer
        case 'Yes'
            [FilenamePreset, PathPreset]  = uigetfile('*.mat','Select preset file','e:\Projects\LNOF\BehaviorData\4_Preset\');
        case 'No'
...
for part=1:BodyPartsNumber
    ...
    % Шаг 1: Анализ данных
    coordinatesX = BodyPartsTraces(part).TraceOriginal.X  ;
    coordinatesY = BodyPartsTraces(part).TraceOriginal.Y  ;

    % Шаг 2: Автоматический подбор параметров
    params = auto_tune_kalman(coordinatesX);

    % Шаг 3: Запуск с подобранными параметрами
    [outliersX, ~, ~] = detect_outliers_kalman(coordinatesX, ...
        'ProcessNoise', params.Q, ...
        'ObservationNoise', params.R, ...
        'Threshold', params.threshold, ...
        'ModelOrder', params.model_order);
    
    [outliersY, ~, ~] = detect_outliers_kalman(coordinatesY, ...
    'ProcessNoise', params.Q, ...
    'ObservationNoise', params.R, ...
    'Threshold', params.threshold, ...
    'ModelOrder', params.model_order);
    ...
    TempArrayX(outliersX) = 0;
    TempArrayY(outliersY) = 0;
