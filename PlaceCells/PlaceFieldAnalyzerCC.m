function [FieldsIC] = PlaceFieldAnalyzerCC(path, filename,pathNV, filenameNV)
...
bin_size_sm = 10; %length of bin in sm
...
if nargin<4
    ...
    %loading spike file
    [filenameNV, pathNV]  = uigetfile('*.csv','load the spike file','d:\_WORK\CC\Spikes\');
...
time_min = 0.0018*TimeTotal; %time in minutes/seconds for minimum summary time in sectors
...
h = figure('Position', [1 1 Screensize(3) Screensize(4)]);
DrawHeatMapMod (1,1,1,0,0,0, N_time_sm, 0, x_int_sm, y_int_sm, bin_size, x_kcorr,spike_t_good);
...
for i=g_cell       
    ...
    if Plot_Spike
        h = figure('Position', [1 1 Screensize(3) Screensize(4)]);
        DrawHeatMapMod (0,1,1,0,0,1, N, max_N, x_int_sm, y_int_sm, bin_size, x_kcorr,spike_t_good);
    ...
    if Plot_Spike_Smooth
        h = figure('Position', [1 1 Screensize(3) Screensize(4)]);
        DrawHeatMapMod (0,1,1,0,0,1, N_sm, max_N_sm, x_int_sm, y_int_sm, bin_size, x_kcorr,spike_t_good);
    ...
    if Plot_FiringRate
        if Cell_IC(2,i)
            h = figure('Position', [1 1 Screensize(3) Screensize(4)]);             
            DrawHeatMapMod (0,1,1,0,0,1, N_freq, max_N_freq, x_int_sm, y_int_sm, bin_size, x_kcorr,spike_t_good);
            title(sprintf('Firing rate of informative cell %d (#/min)', i), 'FontSize', FontSizeTitle);
            F = getframe(h);
            imwrite(F.cdata, sprintf('%s\\Heatmap_FiringRate_Informative\\%s_Heatmap_FiringRate_Informative_%d.png', path, FilenameOut,i));         
            delete(h); 
    ...
    if Plot_FiringRate_Smooth
        h = figure('Position', [1 1 Screensize(3) Screensize(4)]);
        DrawHeatMapMod (0,1,1,0,0,1, N_freq_filt, max_N_freq_filt, x_int_sm, y_int_sm, bin_size, x_kcorr,spike_t_good);
    ...
    if Plot_FiringRate_Smooth_Thres
        if Cell_IC(2,i)
            h = figure('Position', [1 1 Screensize(3) Screensize(4)]);
            DrawHeatMapMod (0,1,1,0,0,1, N_freq_filt_norm_thres, 0, x_int_sm, y_int_sm, bin_size, x_kcorr,spike_t_good);
...
h = figure('Position', [1 1 Screensize(3) Screensize(4)]);
DrawHeatMapMod (1,1,1,0,0,0, N_sum, 0, x_int_sm, y_int_sm, bin_size, x_kcorr,spike_t_good);
...
h = figure('Position', [1 1 Screensize(3) Screensize(4)]);
DrawHeatMapMod (1,1,1,0,0,0, N_freq_filt_sum, 0, x_int_sm, y_int_sm, bin_size, x_kcorr,spike_t_good);
...
h = figure('Position', [1 1 Screensize(3) Screensize(4)]);
DrawHeatMapMod (1,1,1,0,0,0, N_freq_filt_norm_sum, 0, x_int_sm, y_int_sm, bin_size, x_kcorr,spike_t_good);
...
for map = g_cell
    ...
    for mask_field=1:n_wfield 
        ...
        if Plot_FiringRate_Fields
            h = figure('Position', [1 1 Screensize(3) Screensize(4)]);
            DrawHeatMapMod (0,1,1,0,0,1, N_freq_filt_true, max_N_freq_filt_norm_thres, x_int_sm, y_int_sm, bin_size, x_kcorr,[spike_in_field{mask_field,:}]);
        ...
        if Plot_FiringRate_Fields_Corrected
            if Fields(9,wfields-n_wfield+mask_field)
                h = figure('Position', [1 1 Screensize(3) Screensize(4)]);
                DrawHeatMapMod (0,1,1,0,0,1, N_freq_filt_norm_thres, max_N_freq_filt_norm_thres, x_int_sm, y_int_sm, bin_size, x_kcorr, [spike_in_field{mask_field,:}]);
        ...
        if Plot_WaterShedField            
            h = figure('Position', [1 1 Screensize(3) Screensize(4)]);
            DrawHeatMapMod (0,1,1,0,0,1, double(mask_wfield(:,:,mask_field)), 0, x_int_sm, y_int_sm, bin_size, x_kcorr, [spike_in_field{mask_field,:}]);
    ...
    if Plot_WaterShed
        h = figure('Position', [1 1 Screensize(3) Screensize(4)]);             
        DrawHeatMapMod (0,1,1,0,0,1, double(L), 0, x_int_sm, y_int_sm, bin_size, x_kcorr, [spike_in_field{mask_field,:}]);
...
h = figure('Position', [1 1 Screensize(3) Screensize(4)]);
DrawHeatMapMod (1,1,1,0,0,0, Field_thres_sum, 0, x_int_sm, y_int_sm, bin_size, x_kcorr,spike_t_good);
...
h = figure('Position', [1 1 Screensize(3) Screensize(4)]);
DrawHeatMapMod (1,1,1,0,0,0, Field_thres_norm_sum, 0, x_int_sm, y_int_sm, bin_size, x_kcorr,spike_t_good);
...
h = figure('Position', [1 1 Screensize(3) Screensize(4)]);
DrawHeatMapMod (1,1,1,0,0,0, Field_thres_norm_sum_IC, 0, x_int_sm, y_int_sm, bin_size, x_kcorr,spike_t_good);
...
h = figure('Position', [1 1 Screensize(3) Screensize(4)]);
DrawHeatMapMod (1,1,1,0,0,0, Field_thres_sum_NOT_IC, 0, x_int_sm, y_int_sm, bin_size, x_kcorr,spike_t_good);
...
h = figure('Position', [1 1 Screensize(3) Screensize(4)]);
DrawHeatMapMod (1,1,1,0,0,0, Field_thres_sum_IC, 0, x_int_sm, y_int_sm, bin_size, x_kcorr,spike_t_good);
...
h = figure('Position', [1 1 Screensize(3) Screensize(4)]);
DrawHeatMapMod (1,1,1,0,0,0, Field_thres_norm_sum_NOT_IC, 0, x_int_sm, y_int_sm, bin_size, x_kcorr,spike_t_good);
...
%         if Plot_Field
%             h = figure('Position', [1 1 Screensize(3) Screensize(4)]);
%             DrawHeatMapMod (1,1,1,0,0,0, MapFieldsIC(:,:,field),max_N_freq_filt_norm_thres, x_int_sm, y_int_sm, bin_size, x_kcorr,spike_t_good);
...
%         hold on;plot(x_arena,y_arena, 'k');
        ...
        hold on;plot((xrealms+0.5)*bin_size,(yrealms+0.5)*bin_size,'k*','MarkerSize',5,'LineWidth',LineWidthSpikes);
