function [FieldsIC] = PlaceFieldAnalyzer(path, filename, filenameNV, filename_V, start, app, endd, n_objects, field_method)
...
SmoothWindowS = 0.05; % in seconds 0.5 usual
% x_kcorr = 4/3;  %correction coefficient for cage space (X axe)
x_kcorr = 1;  % for good traces
...
% pxl2sm = 95/4; %pixels in sm (for odor and holes arena), y axes
pxl2sm = 1; %pixels in sm (for odor and holes arena), y axes
bin_size_sm = 2.5; %length of bin in sm
bin_size = pxl2sm*bin_size_sm; %length in pixels for bin size
min_spike = 3; %minimum number of spikes for active cell
min_good_line = 3; %minimum number of line with spikes inside a field
k_pp = 0; %percentage of good entries for field candidate
area_k = 1.5; %area scale for cup fields

vel_border = 2; % velocity border in sm/s
...
S_sigma = 1; %criteria for informative place cell(~95%)
...
if nargin<9
    ...
    [filename, path]  = uigetfile('*.csv','load the VT file','d:\_WORK\CC\Features\');
    
    %loading spike file
    [filenameNV, ~]  = uigetfile('*.csv','load the spike file','d:\_WORK\CC\Spikes\');
...
fileID = fopen(sprintf('%s%s', path, filename_V),'r');
Coords = fscanf(fileID,'%d');
fclose(fileID);
...
num_dir = 1;
while isdir(sprintf('%s\\%s_%d', path, path_folder, num_dir))
	num_dir = num_dir+1; 
end
...
switch n_objects 
    case  2
        cup1_centr_x = Coords(1);cup1_centr_y = Coords(2);
        cup2_centr_x = Coords(3);cup2_centr_y = Coords(4);
        cup1_rad = Coords(5);cup2_rad = Coords(6);
        x_arena = Coords(7:11);y_arena = Coords(12:16);
        cup3_centr_x = cup2_centr_x; cup3_centr_y = cup2_centr_y;
        cup3_rad = cup2_rad;
    case 1
        cup1_centr_x = Coords(1);cup1_centr_y = Coords(2);
        cup2_centr_x = Coords(3);cup2_centr_y = Coords(4);
        cup3_centr_x = Coords(5);cup3_centr_y = Coords(6);
        cup1_rad = Coords(7);cup2_rad = Coords(8);cup3_rad = Coords(9);
        x_arena = Coords(10:14);y_arena = Coords(15:19);
    case 0
        % for space without objects (Fear Conditioning)
        cup1_centr_x = 0;cup1_centr_y = 0;
        cup2_centr_x = 0;cup2_centr_y = 0;
        cup3_centr_x = 0;cup3_centr_y = 0;
        cup1_rad = 0.001;cup2_rad = 0.001;cup3_rad = 0.001;
        x_arena = [0 29 29 0 0];y_arena = [0 0 24 24 0];
...
time_min = 0.0035*TimeTotal; %time in minutes/seconds for minimum summary time in sectors

NV_start = app-start+1;
file_NV = file_NV_bad(NV_start:NV_start+n_frames-1,:);
n_cells = size(file_NV, 2)-1;

x_nan = find(isnan(x_bad)); y_nan = find(isnan(y_bad));%array of NaN in x_bad and y_bad
...
h=figure;
plot(time,vel);hold on;plot(time,vel_sm,'r');hold on;plot(time,vel_ref*vel_border,'g'); 
...
t = [0:pi/180:2*pi];
%searching of line in cup1 area
cup1_line(1:n_frames) = 0;
for i=1:n_frames
    if sqrt((x_int_sm(i)-cup1_centr_x)^2+(y_int_sm(i)-cup1_centr_y)^2) <=cup1_rad
        cup1_line(i) = 1;
    end
end
[cup1_line_ref, n_entries_cup1, time_cup1,~,~,~] = RefineLine(cup1_line, length_line, length_line);
x_cup1=cup1_centr_x+cup1_rad*cos(t);y_cup1=cup1_centr_y+cup1_rad*sin(t); 

%searching of line in cup2 area
cup2_line(1:n_frames) = 0;
for i=1:n_frames
    if sqrt((x_int_sm(i)-cup2_centr_x)^2+(y_int_sm(i)-cup2_centr_y)^2) <=cup2_rad
        cup2_line(i) = 1;
    end
end
[cup2_line_ref, n_entries_cup2, time_cup2,~,~,~] = RefineLine(cup2_line, length_line, length_line);
x_cup2=cup2_centr_x+cup2_rad*cos(t);y_cup2=cup2_centr_y+cup2_rad*sin(t);

%searching of line in cup3 area
if n_objects == 3
    cup3_line(1:n_frames) = 0;
    for i=1:n_frames
        if sqrt((x_int_sm(i)-cup3_centr_x)^2+(y_int_sm(i)-cup3_centr_y)^2) <=cup3_rad
            cup3_line(i) = 1;
        end
    end
    [cup3_line_ref, n_entries_cup3, time_cup3,~,~,~] = RefineLine(cup3_line, length_line, length_line);
    x_cup3=cup3_centr_x+cup3_rad*cos(t);y_cup3=cup3_centr_y+cup3_rad*sin(t);
else
    cup3_line = cup2_line;
    cup3_line_ref = cup2_line_ref;
    x_cup3 = x_cup2;
    y_cup3 = y_cup2;
end

ax_xy(2) = max([x_arena(2) x_arena(3)  max(x_int_sm)])+axes_step;
ax_xy(1) = min([x_arena(1) x_arena(4)  min(x_int_sm)])-axes_step;
ax_xy(4) = max([y_arena(4) y_arena(3)  max(y_int_sm) max(y_cup1) max(y_cup2)])+axes_step;
ax_xy(3) = min([y_arena(1) y_arena(2)  min(y_int_sm) min(y_cup1) min(y_cup2)])-axes_step;

if vel_opt
   velcam = vel_ref;
   x_vel_b = x_int_sm.*velcam';
   y_vel_b = y_int_sm.*velcam';
else
   velcam(1:n_frames) = 1;
   x_vel_b = x_int_sm;
   y_vel_b = y_int_sm;
...
if Plot_Single_Spike
    for i=1:n_cells
        ...
        if length(spike_t) >= min_spike
            spike_t_good = round(spike_t);
            h = figure('Position', [1 1 Screensize(3) Screensize(4)]);
%             h = figure;
            axis([ax_xy(1) ax_xy(2) ax_xy(3) ax_xy(4)]);
            title(sprintf('Trajectory of mouse with n = %d Ca2+ events of cell #%d', length(spike_t_good), i), 'FontSize', FontSizeTitle);
            ...
            hold on;DrawLine(x_int_sm, y_int_sm, cup1_line_ref.*velcam, 1, 'r', 0, 1); 
            hold on;DrawLine(x_int_sm, y_int_sm, cup2_line_ref.*velcam, 1, 'c', 0, 1);
            hold on;plot(x_vel_b(spike_t_good),y_vel_b(spike_t_good),'k*', 'MarkerSize',MarksizeSpikes, 'LineWidth',LineWidthSpikes);
            hold on;plot(x_arena,y_arena, 'k', 'LineWidth',LineWidthObjects); 
            hold on;plot(x_cup1,y_cup1, 'r', 'LineWidth',LineWidthObjects);
            hold on;plot(x_cup2,y_cup2, 'c', 'LineWidth',LineWidthObjects);
            if n_objects == 3 
                hold on;plot(x_cup3,y_cup3, 'b', 'LineWidth',LineWidthObjects);
                hold on;DrawLine(x_int_sm, y_int_sm, cup3_line_ref.*velcam, 1, 'b', 0, 1);
            end
            F = getframe(h);
            imwrite(F.cdata, sprintf('%s\\Single_spikes\\%s_single_spikes_%d.png',path,FilenameOut,i));           
            delete(h);
        end
    end
end
...
hold on;plot(x_cup1,y_cup1, 'r', 'LineWidth',3);
hold on;plot(x_cup2,y_cup2, 'c', 'LineWidth',3); 
if n_objects == 3 
    hold on;plot(x_cup3,y_cup3, 'b', 'LineWidth',3);
    hold on;DrawLine(x_int_sm, y_int_sm, cup3_line_ref.*velcam, 1, 'b', 0, 1);
end
...
for i=1:n_cells
    ...
    if length(spike_t) >= min_spike
        spike_t_good = round(spike_t);
        hold on;plot(x_vel_b(spike_t_good),y_vel_b(spike_t_good),'k*', 'MarkerSize',MarksizeSpikesAll, 'LineWidth',LineWidthSpikes);
    end
end
F = getframe(h);
imwrite(F.cdata, sprintf('%s\\%s_spike_all_plot.png',path,FilenameOut)); 
delete(h);

% %animation video plot
% sum_s = sum(file_NV(:,2:n_cells+1)');
% h = figure;
% title('Trajectory of mouse with all Ca2+ events','FontSize', 15);
% axis([ax_xy(1) ax_xy(2) ax_xy(3) ax_xy(4)]);
% hold on;plot(x_cup1,y_cup1, 'r','LineWidth',2);
% hold on;plot(x_cup2,y_cup2, 'c','LineWidth',2);
% hold on;plot(x_arena,y_arena, 'k','LineWidth',2);
% for k=1:length(sum_s)
%     hold on;plot(x_int_sm(k),y_int_sm(k), 'g.','MarkerSize',5);   
%     if sum_s(k)>0
%         hold on;plot(x_int_sm(k),y_int_sm(k),'r*','MarkerSize',10);     
%     end
%     pause(0.04);
% end
...
N_frame_orig(1:SizeMY,1:SizeMX) = 0;
for d=1:n_frames
    N_frame_orig(y_ind(d),x_ind(d)) = N_frame_orig(y_ind(d),x_ind(d))+1;
end

N_time_orig = N_frame_orig/FrameRate/TimeRate; %in minutes/seconds
N_time_with_min = N_time_orig;
N_time_with_min(find(N_time_orig<time_min)) = 0;

[N_time_sm,mask_t] = ConvBorderFix(N_time_with_min,0);
mask_s = double(mask_t == 0);

%heatmap for occupancy map
h = figure('Position', [1 1 Screensize(3) Screensize(4)]);
DrawHeatMapMod (n_objects,1,1,1,0,0,0, N_time_sm, 0, cup1_centr_x, cup1_centr_y, cup1_rad, cup2_centr_x, cup2_centr_y, cup2_rad, cup3_centr_x, cup3_centr_y, cup3_rad, x_arena, y_arena, x_int_sm, y_int_sm, bin_size, x_kcorr,cup1_line_ref,cup2_line_ref, cup3_line_ref,spike_t_good);
title(sprintf('Occupancy map smoothed (%s)',TimeMode), 'FontSize', FontSizeTitle);
F = getframe(h);
imwrite(F.cdata, sprintf('%s\\%s_Heatmap_time_sm.png', path, FilenameOut))   
delete(h); 
...
for i=1:n_cells      
    ...
    if length(spike_t) >= min_spike
        ...
        N_freq_filt_norm_thres(1:SizeMY,1:SizeMX) = 0;  
        ...
        [N_freq_filt, ~] = ConvBorderFix(N_freq,mask_t);
        ...
        mean_N_freq_filt(i) = mean(N_freq_filt(find(N_freq_filt>0))); 
        
%         hh = figure('Position', [1 1 Screensize(3) Screensize(4)]);             
%         DrawHeatMapMod (n_objects,0,1,1,0,0,0, N_freq, 0, cup1_centr_x, cup1_centr_y, cup1_rad, cup2_centr_x, cup2_centr_y, cup2_rad, cup3_centr_x, cup3_centr_y, cup3_rad, x_arena, y_arena, x_int_sm, y_int_sm, bin_size, x_kcorr,cup1_line_ref,cup2_line_ref,cup3_line_ref,spike_t_good);
%         title(sprintf('Firing rate of informative cell %d (#/min)', i), 'FontSize', FontSizeTitle);
%         F = getframe(hh);
%         imwrite(F.cdata, sprintf('%s\\Heatmap_FiringRate_Informative\\%s_Heatmap_FiringRate_Informative_%d.png', path, FilenameOut,i));         
%         delete(hh); 
        ...
        %IC calculation
        Cell_IC(2:5,i) = RandomShiftMod(spike_t_good, x_ind, y_ind, mask_s, mask_t, N_time_sm, N_shift, shift, S_sigma, TimeRate, FrameRate);
...
if ~isempty(mean_N_freq_filt)
    h = figure;
    histogram(mean_N_freq_filt(find(mean_N_freq_filt>0)),50);
    title('Histogram of mean firing rate(#/min)', 'FontSize', FontSizeTitle);
    F = getframe(h);
    imwrite(F.cdata, sprintf('%s\\%s_Histogram_FiringRate.png', path,FilenameOut));         
    delete(h); 
...
MapCells = zeros(SizeMY,SizeMX,n_cells); %thresholded activity maps for all cells
...
for i=g_cell       
    ...
    if Plot_Spike
        h = figure('Position', [1 1 Screensize(3) Screensize(4)]);
        DrawHeatMapMod (n_objects,0,1,1,0,0,1, N, max_N, cup1_centr_x, cup1_centr_y, cup1_rad, cup2_centr_x, cup2_centr_y, cup2_rad, cup3_centr_x, cup3_centr_y, cup3_rad, x_arena, y_arena, x_int_sm, y_int_sm, bin_size, x_kcorr,cup1_line_ref,cup2_line_ref,cup3_line_ref,spike_t_good);
        title(sprintf('Spikes number of cell #%d. Spikes: %d',i, length(spike_t)), 'FontSize', FontSizeTitle);
        F = getframe(h);
        imwrite(F.cdata, sprintf('%s\\Heatmap_Spike\\%s_Heatmap_Spike_%d.png', path,FilenameOut,i));         
        delete(h);
    end

    if Plot_Spike_Smooth
        h = figure('Position', [1 1 Screensize(3) Screensize(4)]);
        DrawHeatMapMod (n_objects,0,1,1,0,0,1, N_sm, max_N_sm, cup1_centr_x, cup1_centr_y, cup1_rad, cup2_centr_x, cup2_centr_y, cup2_rad, cup3_centr_x, cup3_centr_y, cup3_rad, x_arena, y_arena, x_int_sm, y_int_sm, bin_size, x_kcorr,cup1_line_ref,cup2_line_ref,cup3_line_ref,spike_t_good);
        title(sprintf('Spikes number of cell %d (smoothed). Spikes: %d',i, length(spike_t)), 'FontSize', FontSizeTitle);
        F = getframe(h);
        imwrite(F.cdata, sprintf('%s\\Heatmap_Spike_Smooth\\%s_Heatmap_Spike_sm_%d.png', path,FilenameOut,i));         
        delete(h);
    end       
    ...
    if Plot_FiringRate
        if Cell_IC(2,i)
            h = figure('Position', [1 1 Screensize(3) Screensize(4)]);             
            DrawHeatMapMod (n_objects,0,1,1,0,0,1, N_freq, max_N_freq, cup1_centr_x, cup1_centr_y, cup1_rad, cup2_centr_x, cup2_centr_y, cup2_rad, cup3_centr_x, cup3_centr_y, cup3_rad, x_arena, y_arena, x_int_sm, y_int_sm, bin_size, x_kcorr,cup1_line_ref,cup2_line_ref,cup3_line_ref,spike_t_good);
            title(sprintf('Firing rate of informative cell %d (#/min)', i), 'FontSize', FontSizeTitle);
            F = getframe(h);
            imwrite(F.cdata, sprintf('%s\\Heatmap_FiringRate_Informative\\%s_Heatmap_FiringRate_Informative_%d.png', path, FilenameOut,i));         
            delete(h); 
            ...
            F = getframe(h);
            imwrite(F.cdata, sprintf('%s\\Heatmap_FiringRate_NOT_Informative\\%s_Heatmap_FiringRate_NOT_Informative_%d.png', path, FilenameOut,i));         
            delete(h); 
        end
    end
    ...
    if Plot_FiringRate_Smooth
        h = figure('Position', [1 1 Screensize(3) Screensize(4)]);
        DrawHeatMapMod (n_objects,0,1,1,0,0,1, N_freq_filt, max_N_freq_filt, cup1_centr_x, cup1_centr_y, cup1_rad, cup2_centr_x, cup2_centr_y, cup2_rad, cup3_centr_x, cup3_centr_y, cup3_rad, x_arena, y_arena, x_int_sm, y_int_sm, bin_size, x_kcorr,cup1_line_ref,cup2_line_ref,cup3_line_ref,spike_t_good);
        title(sprintf('Firing rate (smoothed) of cell %d (#/min)', i), 'FontSize', FontSizeTitle);
        F = getframe(h);
        imwrite(F.cdata, sprintf('%s\\Heatmap_FiringRate_Smooth\\%s_Heatmap_FiringRate_sm_%d.png', path,FilenameOut,i));             
        delete(h);
    end
    ...
    if Plot_FiringRate_Smooth_Thres
        if Cell_IC(2,i)
            h = figure('Position', [1 1 Screensize(3) Screensize(4)]);
            DrawHeatMapMod (n_objects,0,1,1,0,0,1, N_freq_filt_norm_thres, max_N_freq_filt_norm_thres, cup1_centr_x, cup1_centr_y, cup1_rad, cup2_centr_x, cup2_centr_y, cup2_rad, cup3_centr_x, cup3_centr_y, cup3_rad, x_arena, y_arena, x_int_sm, y_int_sm, bin_size, x_kcorr,cup1_line_ref,cup2_line_ref,cup3_line_ref,spike_t_good);
            title(sprintf('Firing rate of informative cell %d (smoothed and thresholded)(#/min)',i), 'FontSize', FontSizeTitle);
            F = getframe(h);
            imwrite(F.cdata, sprintf('%s\\Heatmap_FiringRate_Smooth_Thres_Informative\\%s_Heatmap_FiringRate_sm_thres_Informative_%d.png', path,FilenameOut,i));         
            delete(h);
        else
            h = figure('Position', [1 1 Screensize(3) Screensize(4)]);
            DrawHeatMapMod (n_objects,0,1,1,0,0,1, N_freq_filt_norm_thres, max_N_freq_filt_norm_thres, cup1_centr_x, cup1_centr_y, cup1_rad, cup2_centr_x, cup2_centr_y, cup2_rad, cup3_centr_x, cup3_centr_y, cup3_rad, x_arena, y_arena, x_int_sm, y_int_sm, bin_size, x_kcorr,cup1_line_ref,cup2_line_ref,cup3_line_ref,spike_t_good);
            ...
            F = getframe(h);
            imwrite(F.cdata, sprintf('%s\\Heatmap_FiringRate_Smooth_Thres_NOT_Informative\\%s_Heatmap_FiringRate_sm_thres_NOT_Informative_%d.png', path,FilenameOut,i));         
            delete(h);
        end
    end
end
...
h = figure('Position', [1 1 Screensize(3) Screensize(4)]);
DrawHeatMapMod (n_objects,1,1,1,0,0,0, N_sum, 0, cup1_centr_x, cup1_centr_y, cup1_rad, cup2_centr_x, cup2_centr_y, cup2_rad, cup3_centr_x, cup3_centr_y, cup3_rad, x_arena, y_arena, x_int_sm, y_int_sm, bin_size, x_kcorr,cup1_line_ref,cup2_line_ref,cup3_line_ref,spike_t_good);
title('Sum of spikes map', 'FontSize', FontSizeTitle);
F = getframe(h);
imwrite(F.cdata, sprintf('%s\\%s_Heatmap_AllCells_spikes.png', path, FilenameOut));     
delete(h);  

h = figure('Position', [1 1 Screensize(3) Screensize(4)]);
DrawHeatMapMod (n_objects,1,1,1,0,0,0, N_freq_filt_sum, 0, cup1_centr_x, cup1_centr_y, cup1_rad, cup2_centr_x, cup2_centr_y, cup2_rad, cup3_centr_x, cup3_centr_y, cup3_rad, x_arena, y_arena, x_int_sm, y_int_sm, bin_size, x_kcorr,cup1_line_ref,cup2_line_ref,cup3_line_ref,spike_t_good);
title('Firing rate for all cells(#/min)', 'FontSize', FontSizeTitle);
F = getframe(h);
imwrite(F.cdata, sprintf('%s\\%s_Heatmap_AllCells_FiringRate.png', path, FilenameOut));     
delete(h);  

h = figure('Position', [1 1 Screensize(3) Screensize(4)]);
DrawHeatMapMod (n_objects,1,1,1,0,0,0, N_freq_filt_norm_sum, 0, cup1_centr_x, cup1_centr_y, cup1_rad, cup2_centr_x, cup2_centr_y, cup2_rad, cup3_centr_x, cup3_centr_y, cup3_rad, x_arena, y_arena, x_int_sm, y_int_sm, bin_size, x_kcorr,cup1_line_ref,cup2_line_ref,cup3_line_ref,spike_t_good);
title('Firing rate for all cells normalized (#/min)', 'FontSize', FontSizeTitle);
F = getframe(h);
imwrite(F.cdata, sprintf('%s\\%s_Heatmap_AllCells_Normalized_FiringRate.png', path, FilenameOut));     
delete(h);  
...
for map = g_cell
    ...
    for mask_field=1:n_wfield 
        ...
        switch field_method
            case 'Peak'
                ...
                Fields(9,wfields-n_wfield+mask_field) = (Fields(8,wfields-n_wfield+mask_field)>2)*Fields(3,wfields-n_wfield+mask_field);
            case 'IC_orig'
                Fields(3:7,wfields-n_wfield+mask_field) = Cell_IC(2:6,map);
                Fields(9,wfields-n_wfield+mask_field) = (Fields(8,wfields-n_wfield+mask_field)>2)*Fields(3,wfields-n_wfield+mask_field);
            case 'IC_multi'
                Fields(3:6,wfields-n_wfield+mask_field) = RandomShiftMod([spike_in_field{mask_field,:}], x_ind, y_ind, mask_s, N_time_sm, N_shift, shift, S_sigma);
        ...
        if Plot_FiringRate_Fields
            h = figure('Position', [1 1 Screensize(3) Screensize(4)]);
            DrawHeatMapMod (n_objects,0,1,1,0,0,1, N_freq_filt_true, max_N_freq_filt_norm_thres, cup1_centr_x, cup1_centr_y, cup1_rad, cup2_centr_x, cup2_centr_y, cup2_rad, cup3_centr_x, cup3_centr_y, cup3_rad, x_arena, y_arena, x_int_sm, y_int_sm, bin_size, x_kcorr,cup1_line_ref,cup2_line_ref,cup3_line_ref,[spike_in_field{mask_field,:}]);
            ...
            F = getframe(h);
            imwrite(F.cdata, sprintf('%s\\Heatmap_FiringRate_Fields\\%s_FiringRateFields_%d.png', path,FilenameOut,wfields-n_wfield+mask_field));             
            delete(h);
        end   
        ...
        if Plot_FiringRate_Fields_Corrected
            if Fields(9,wfields-n_wfield+mask_field)
                h = figure('Position', [1 1 Screensize(3) Screensize(4)]);
                DrawHeatMapMod (n_objects,0,1,1,0,0,1, N_freq_filt_norm_thres, max_N_freq_filt_norm_thres, cup1_centr_x, cup1_centr_y, cup1_rad, cup2_centr_x, cup2_centr_y, cup2_rad, cup3_centr_x, cup3_centr_y, cup3_rad, x_arena, y_arena, x_int_sm, y_int_sm, bin_size, x_kcorr,cup1_line_ref,cup2_line_ref,cup3_line_ref, [spike_in_field{mask_field,:}]);
                title(sprintf('Firing rate of informative field %d of cell %d (smoothed and thresholded)(#/min) \n IC = %.2f, MU = %.3f, SIGMA = %.3f, Nsig = %.1f',mask_field,map,Fields(4:7,wfields-n_wfield+mask_field)), 'FontSize', FontSizeTitle);
                F = getframe(h);
                imwrite(F.cdata, sprintf('%s\\Heatmap_FiringRate_Fields_Corrected_Inform\\%s_FiringRate_Fields_Corrected_Inform_%d.png', path,FilenameOut,wfields-n_wfield+mask_field));         
                delete(h);
            else
                h = figure('Position', [1 1 Screensize(3) Screensize(4)]);
                DrawHeatMapMod (n_objects,0,1,1,0,0,1, N_freq_filt_norm_thres, max_N_freq_filt_norm_thres, cup1_centr_x, cup1_centr_y, cup1_rad, cup2_centr_x, cup2_centr_y, cup2_rad, cup3_centr_x, cup3_centr_y, cup3_rad, x_arena, y_arena, x_int_sm, y_int_sm, bin_size, x_kcorr,cup1_line_ref,cup2_line_ref,cup3_line_ref,[spike_in_field{mask_field,:}]);
                ...
                F = getframe(h);
                imwrite(F.cdata, sprintf('%s\\Heatmap_FiringRate_Fields_Corrected_NOT_Inform\\%s_FiringRate_Fields_Corrected_NOT_Inform_%d.png', path,FilenameOut,wfields-n_wfield+mask_field));         
                delete(h);
            end  
        end    
        ...
        if Plot_WaterShedField            
            h = figure('Position', [1 1 Screensize(3) Screensize(4)]);
            DrawHeatMapMod (n_objects,0,1,1,0,0,1, double(mask_wfield(:,:,mask_field)), 0, cup1_centr_x, cup1_centr_y, cup1_rad, cup2_centr_x, cup2_centr_y, cup2_rad, cup3_centr_x, cup3_centr_y, cup3_rad, x_arena, y_arena, x_int_sm, y_int_sm, bin_size, x_kcorr,cup1_line_ref,cup2_line_ref,cup3_line_ref,[spike_in_field{mask_field,:}]);
            title(sprintf('WaterShed Transform of cell %d field %d ICcrit= %d \n IC = %.2f, MU = %.3f, SIGMA = %.3f, Nsig = %.1f', map, mask_field,Fields(3:7,wfields-n_wfield+mask_field)), 'FontSize', FontSizeTitle);
            F = getframe(h);
            imwrite(F.cdata, sprintf('%s\\WaterShedFields\\%s_WaterShedField_%d.png', path,FilenameOut,wfields-n_wfield+mask_field));             
            delete(h);            
        end
    end 
    ...
    if Plot_WaterShed
        h = figure('Position', [1 1 Screensize(3) Screensize(4)]);             
        DrawHeatMapMod (n_objects,0,1,1,0,0,1, double(L), 0, cup1_centr_x, cup1_centr_y, cup1_rad, cup2_centr_x, cup2_centr_y, cup2_rad, cup3_centr_x, cup3_centr_y, cup3_rad, x_arena, y_arena, x_int_sm, y_int_sm, bin_size, x_kcorr,cup1_line_ref,cup2_line_ref,cup3_line_ref,spike_t_good);
        ...
        F = getframe(h);
        imwrite(F.cdata, sprintf('%s\\WaterShed\\%s_WaterShed_%d.png', path,FilenameOut,map));
        delete(h);
    end
end
   
h = figure('Position', [1 1 Screensize(3) Screensize(4)]);
DrawHeatMapMod (n_objects,1,1,1,0,0,0, Field_thres_sum, 0, cup1_centr_x, cup1_centr_y, cup1_rad, cup2_centr_x, cup2_centr_y, cup2_rad, cup3_centr_x, cup3_centr_y, cup3_rad, x_arena, y_arena, x_int_sm, y_int_sm, bin_size, x_kcorr,cup1_line_ref,cup2_line_ref,cup3_line_ref,spike_t_good);
title('Firing rate for all corrected fields(#/min)', 'FontSize', FontSizeTitle);
F = getframe(h);
imwrite(F.cdata, sprintf('%s\\%s_Heatmap_AllFields_FiringRate.png', path, FilenameOut));     
delete(h);  

h = figure('Position', [1 1 Screensize(3) Screensize(4)]);
DrawHeatMapMod (n_objects,1,1,1,0,0,0, Field_thres_norm_sum, 0, cup1_centr_x, cup1_centr_y, cup1_rad, cup2_centr_x, cup2_centr_y, cup2_rad, cup3_centr_x, cup3_centr_y, cup3_rad, x_arena, y_arena, x_int_sm, y_int_sm, bin_size, x_kcorr,cup1_line_ref,cup2_line_ref,cup3_line_ref,spike_t_good);
title('Normalized firing rate for all corrected fields(#/min)', 'FontSize', FontSizeTitle);
F = getframe(h);
imwrite(F.cdata, sprintf('%s\\%s_Heatmap_AllFields_Normalized_FiringRate.png', path, FilenameOut));     
delete(h); 

h = figure('Position', [1 1 Screensize(3) Screensize(4)]);
DrawHeatMapMod (n_objects,1,1,1,0,0,0, Field_thres_norm_sum_IC, 0, cup1_centr_x, cup1_centr_y, cup1_rad, cup2_centr_x, cup2_centr_y, cup2_rad, cup3_centr_x, cup3_centr_y, cup3_rad, x_arena, y_arena, x_int_sm, y_int_sm, bin_size, x_kcorr,cup1_line_ref,cup2_line_ref,cup3_line_ref,spike_t_good);
title('Firing rate for all inform corrected fields(#/min)', 'FontSize', FontSizeTitle);
F = getframe(h);
imwrite(F.cdata, sprintf('%s\\%s_Heatmap_AllFields_Inform_FiringRate.png', path, FilenameOut));     
delete(h);

h = figure('Position', [1 1 Screensize(3) Screensize(4)]);
DrawHeatMapMod (n_objects,1,1,1,0,0,0, Field_thres_sum_NOT_IC, 0, cup1_centr_x, cup1_centr_y, cup1_rad, cup2_centr_x, cup2_centr_y, cup2_rad, cup3_centr_x, cup3_centr_y, cup3_rad, x_arena, y_arena, x_int_sm, y_int_sm, bin_size, x_kcorr,cup1_line_ref,cup2_line_ref,cup3_line_ref,spike_t_good);
title('Firing rate for all NOT inform corrected fields(#/min)', 'FontSize', FontSizeTitle);
F = getframe(h);
imwrite(F.cdata, sprintf('%s\\%s_Heatmap_AllFields_NOT_Inform_FiringRate.png', path, FilenameOut));     
delete(h);

h = figure('Position', [1 1 Screensize(3) Screensize(4)]);
DrawHeatMapMod (n_objects,1,1,1,0,0,0, Field_thres_sum_IC, 0, cup1_centr_x, cup1_centr_y, cup1_rad, cup2_centr_x, cup2_centr_y, cup2_rad, cup3_centr_x, cup3_centr_y, cup3_rad, x_arena, y_arena, x_int_sm, y_int_sm, bin_size, x_kcorr,cup1_line_ref,cup2_line_ref,cup3_line_ref,spike_t_good);
title('Normalized firing rate for all inform corrected fields(#/min)', 'FontSize', FontSizeTitle);
F = getframe(h);
imwrite(F.cdata, sprintf('%s\\%s_Heatmap_AllFields_norm_Inform_FiringRate.png', path, FilenameOut));     
delete(h);

h = figure('Position', [1 1 Screensize(3) Screensize(4)]);
DrawHeatMapMod (n_objects,1,1,1,0,0,0, Field_thres_norm_sum_NOT_IC, 0, cup1_centr_x, cup1_centr_y, cup1_rad, cup2_centr_x, cup2_centr_y, cup2_rad, cup3_centr_x, cup3_centr_y, cup3_rad, x_arena, y_arena, x_int_sm, y_int_sm, bin_size, x_kcorr,cup1_line_ref,cup2_line_ref,cup3_line_ref,spike_t_good);
title('Normalized firing rate for all NOT inform corrected fields(#/min)', 'FontSize', FontSizeTitle);
F = getframe(h);
imwrite(F.cdata, sprintf('%s\\%s_Heatmap_AllFields_norm_NOT_Inform_FiringRate.png', path, FilenameOut));     
delete(h);
...
if length(Fields(7,:))>2
    ...
    F = getframe(h);
    imwrite(F.cdata, sprintf('%s\\%s_Histogram_IC_fields_normalized.png', path,FilenameOut));         
    delete(h);     
end

if length(Cell_IC(1,:))>2
    h = figure;
    histogram(Cell_IC(6,find(Cell_IC(2,:)>=0)),round(length(Cell_IC(6,:))/5));
    title('Histogram of z-scored IC distribution for cells', 'FontSize', FontSizeLabel);
    F = getframe(h);
    imwrite(F.cdata, sprintf('%s\\%s_Histogram_IC_cells_normalized.png', path,FilenameOut));
    delete(h);
end
...
%         spike_t = find(file_NV(:,SpikeFieldsStruct(field).cell+1));     
%         spike_t_good = round(spike_t);
...
%         if Plot_Field
%             h = figure('Position', [1 1 Screensize(3) Screensize(4)]);
%             DrawHeatMapMod (n_objects,1,1,1,0,0,0, MapFieldsIC(:,:,field),max_N_freq_filt_norm_thres, cup1_centr_x, cup1_centr_y, cup1_rad, cup2_centr_x, cup2_centr_y, cup2_rad, cup3_centr_x, cup3_centr_y, cup3_rad, x_arena, y_arena, x_int_sm, y_int_sm, bin_size, x_kcorr,cup1_line_ref,cup2_line_ref,cup3_line_ref,spike_t_good);
...
%                 imwrite(F.cdata, sprintf('%s\\Heatmap_Fields_Real\\%s_Heatmap_Field_Real_%d.png', path,FilenameOut,field));
%             else
%                 imwrite(F.cdata, sprintf('%s\\Heatmap_Fields_NOT_Real\\%s_Heatmap_Field_NOT_Real_%d.png', path,FilenameOut,field));
%             end
%             delete(h);   
%         end
%     end
...
%         plot(x_int_sm,y_int_sm, 'g');
%         hold on;DrawLine(x_int_sm, y_int_sm, cup1_line_ref, 1, 'r', 0, 1); 
%         hold on;DrawLine(x_int_sm, y_int_sm, cup2_line_ref, 1, 'c', 0, 1);
%         if n_objects == 3
%             hold on;DrawLine(x_int_sm, y_int_sm, cup3_line_ref, 1, 'b', 0, 1);
%             hold on;plot(x_cup3,y_cup3, 'b', 'LineWidth',3);
%         end
%         hold on;plot(x_arena,y_arena, 'k');
%         title('All real fields', 'FontSize', FontSizeTitle);
%         hold on;plot(x_cup1,y_cup1, 'r', 'LineWidth',LineWidthSpikes);
%         hold on;plot(x_cup2,y_cup2, 'c', 'LineWidth',LineWidthSpikes);     
%         hold on;plot(xrealms*bin_size,yrealms*bin_size,'k*','MarkerSize',5,'LineWidth',LineWidthSpikes);
...
% 
%         h = figure('Position', [1 1 Screensize(3) Screensize(4)]);
%         DrawHeatMapMod (n_objects,1,1,1,0,0,0, N_real_fields_sum, 0, cup1_centr_x, cup1_centr_y, cup1_rad, cup2_centr_x, cup2_centr_y, cup2_rad, cup3_centr_x, cup3_centr_y, cup3_rad, x_arena, y_arena, x_int_sm, y_int_sm, bin_size, x_kcorr,cup1_line_ref,cup2_line_ref,cup3_line_ref, spike_t_good);
%         title('Firing rate for all real fields(normalized)', 'FontSize', FontSizeTitle);
%         F = getframe(h);
%         imwrite(F.cdata, sprintf('%s\\%s_Heatmap_AllRealFields.png', path, FilenameOut));     
%         delete(h); 
% 
%         h = figure('Position', [1 1 Screensize(3) Screensize(4)]); 
%         DrawHeatMapMod (n_objects,1,1,1,0,0,0, N_not_real_fields_sum, 0, cup1_centr_x, cup1_centr_y, cup1_rad, cup2_centr_x, cup2_centr_y, cup2_rad, cup3_centr_x, cup3_centr_y, cup3_rad, x_arena, y_arena, x_int_sm, y_int_sm, bin_size, x_kcorr,cup1_line_ref,cup2_line_ref,cup3_line_ref, spike_t_good);
%         title('Firing rate for all not real fields(normalized)', 'FontSize', FontSizeTitle);
%         F = getframe(h);
%         imwrite(F.cdata, sprintf('%s\\%s_Heatmap_AllNotRealFields.png', path, FilenameOut));     
%         delete(h);              
% 
...
%         F = getframe(h);
%         imwrite(F.cdata, sprintf('%s\\%s_Fields per cell distribution.png', path, FilenameOut));         
%         delete(h); 
% 
%         switch n_objects
%             case 2
%                 %some results for output
%                 results(1) = size(SpikeFieldsStruct,2); %total candidate fields
%                 results(2) = size(SpikeFieldsReal,2); %total real fields
%                 results(3) = size(SpikeFieldsReal,2)-cup1_field_s-cup2_field_s; %total real place fields
%                 results(4) = cup1_field_s+cup2_field_s; %total real objects fields
%                 results(5) = cup1_field_s;
%                 results(6) = cup2_field_s;
%                 results(7) = time_cup1/FrameRate;
%                 results(8) = time_cup2/FrameRate ;
%                 results(9) = n_entries_cup1;
%                 results(10) = n_entries_cup2;
%                 r_results = round(results);
% 
%                 %save results
%                 prmtr=fopen(sprintf('%s\\_s_FieldsCupStat.txt',path, FilenameOut),'w');
%                 fprintf(prmtr,'All fields Real fields Place fields Object fields Cup1 fields Cup2 fields Time_cup1  Time_cup2  N_entries_cup1 N_entries_cup2 \n');
%                 fprintf(prmtr, '%d %d %d %d %d %d %d %d %d %d\n',r_results(1), r_results(2), r_results(3), r_results(4), r_results(5), r_results(6), r_results(7), r_results(8), r_results(9), r_results(10));
%                 fclose(prmtr);
%             case 3
%                 %some results for output
%                 results(1) = size(SpikeFieldsStruct,2); %total candidate fields
%                 results(2) = size(SpikeFieldsReal,2); %total real fields
%                 results(3) = size(SpikeFieldsReal,2)-cup1_field_s-cup2_field_s-cup3_field_s; %total real place fields
%                 results(4) = cup1_field_s+cup2_field_s+cup3_field_s; %total real objects fields
%                 results(5) = cup1_field_s;
%                 results(6) = cup2_field_s;
%                 results(7) = cup3_field_s;
%                 results(8) = time_cup1/FrameRate;
%                 results(9) = time_cup2/FrameRate;
%                 results(10) = time_cup3/FrameRate;
%                 results(11) = n_entries_cup1;
%                 results(12) = n_entries_cup2;
%                 results(13) = n_entries_cup3;
%                 r_results = round(results);
% 
%                 %save results
%                 prmtr=fopen(sprintf('%s\\%s_FieldsCupStat.txt',path, FilenameOut),'w');
%                 fprintf(prmtr,'All fields Real fields Place fields Object fields Cup1 fields Cup2 fields Cup3 fields Time_cup1  Time_cup2  Time_cup3 N_entries_cup1 N_entries_cup2 N_entries_cup3 \n');
%                 fprintf(prmtr, '%d %d %d %d %d %d %d %d %d %d %d %d %d\n',r_results(1), r_results(2), r_results(3), r_results(4), r_results(5), r_results(6), r_results(7), r_results(8), r_results(9), r_results(10), r_results(11), r_results(12), r_results(13));
%                 fclose(prmtr);
%             case  0 
%                 results(1) = size(SpikeFieldsStruct,2); %total candidate fields
%                 results(2) = size(SpikeFieldsReal,2); %total real fields
