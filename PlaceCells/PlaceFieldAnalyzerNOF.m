function [FieldsIC] = PlaceFieldAnalyzerNOF(path,filename,pathNV,filenameNV,pathPR,filenamePR,plot_opt)
...
FilenameCut = 13; % cut x symbol from filename for main filename
...
bin_size_sm = 4; %length of bin in sm
...
S_sigma = 4; %criteria for informative place cell(~95%)
...
if nargin<7
    ...
    [filename, path]  = uigetfile('*.mat','load the VT file','w:\Projects\NOF\PlaceCellsData\5.1_matfiles\');
    
    %loading spike file
    [filenameNV, pathNV]  = uigetfile('*.csv','load the spike file','w:\Projects\NOF\PlaceCellsData\8_Spikes\');
    
    %loading preset file
    [filenamePR, pathPR]  = uigetfile('*.mat','load preset file','w:\Projects\NOF\PlaceCellsData\4_Presets\');
...
load(sprintf('%s%s', path, filename),'BodyPartsTracesMainX', 'BodyPartsTracesMainY','Point');
file = [BodyPartsTracesMainX(Point.Nose,:)'/Options.pxl2sm BodyPartsTracesMainY(Point.Nose,:)'/Options.pxl2sm];
...
% x_orig = table2array(file(app:end_track, 1))*x_kcorr;
% y_orig = table2array(file(app:end_track, 2));
...
if Plot_Single_Spike
    for i=1:n_cells_for_analysis
        ...
        if length(spike_t_good) >= min_spike
            ...
            saveas(h, sprintf('%s\\Spikes\\%s_Spikes_Cell_%d.png',path,FilenameOut,i));
            delete(h);
        end
    end
end
