...
for ii = 1 : rows
    for jj = 1 : cols
        if holes_values(ii, jj) > 0            
            ...
            kernel_weights = gauss_kernel_restricted .* mask_border(max(1,ii-pad_size):min(rows,ii+pad_size), max(1,jj-pad_size):min(rows,jj+pad_size));
            ...
            if sum_weights > 0  % Avoid division by zero
                ...
                N_corrected(max(1,ii-pad_size):min(rows,ii+pad_size), max(1,jj-pad_size):min(rows,jj+pad_size)) = ...
                    N_corrected(max(1,ii-pad_size):min(rows,ii+pad_size), max(1,jj-pad_size):min(rows,jj+pad_size)) + redistribution;
            end
        end
    end
end
