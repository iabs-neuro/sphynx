function [Acts, BodyPartsTraces, Point] = BehaviorAnalyzerSalmLab(PathVideo, FilenameVideo, PathDLC, FilenameDLC, PathOut, StartTime, EndTime, PathPreset, FilenamePreset)
...
if nargin<9
    ...
    [FilenameVideo, PathVideo]  = uigetfile('*.*','Select video file','d:\_WORK\Project_DLC_Salm_lab\video\');
    [FilenameDLC, PathDLC]  = uigetfile('*.csv','Select DLC file with body parts','d:\_WORK\Project_DLC_Salm_lab\DLC\');
    PathOut = uigetdir('d:\_WORK\Project_DLC_Salm_lab\Behavior\', 'Pick a Directory for Outputs');
    
    % loading preset file
    answer = questdlg('Do you have preset file?', 'Uploading files', 'Yes','No','Yes');
    switch answer
        case 'Yes'
            [FilenamePreset, PathPreset]  = uigetfile('*.mat','Select preset file','d:\_WORK\Project_DLC_Salm_lab\Preset\');
        case 'No'
...
% % for session with 2 objects
% ZonesOption.NameZone = {'ArenaCornersAllRealOut' 'ArenaWallsAllRealOut' 'Center' 'Object1RealOut' 'Object2RealOut' 'ObjectAllRealOut'};
% ZonesOption.NameBodyPart = {'bodycenter' 'bodycenter' 'bodycenter' 'headcenter' 'headcenter' 'headcenter' };
% ZonesOption.NameAct = {'corners' 'walls' 'center' 'object1' 'object2' 'objects'};
% ZonesOption.NumBodyPart = [12 12 12 1 1 1 ];

%  for session w\o objects
...
ZonesOption.NumBodyPart = [12 12 12];
...
for line = 1:size(Acts,2)
    [~, Acts(line).ActNumber, Acts(line).ActPercent, Acts(line).ActDistr,Acts(line).FrameIn,~] = RefineLine(Acts(line).ActArrayRefine, 0,0);
    Acts(line).FrameIn = round(Acts(line).FrameIn/Options.FrameRate,1);
