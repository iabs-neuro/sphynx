function [Acts, BodyPartsTraces, Point] = BehaviorAnalyzerBOF26(PathVideo, FilenameVideo, PathDLC, FilenameDLC, PathOut, StartTime, EndTime, PathPreset, FilenamePreset)
...
if nargin<9
    ...
    [FilenameVideo, PathVideo]  = uigetfile('*.*','Select video file','e:\Projects\BOF\BehaviorData\2_Combined\');
    [FilenameDLC, PathDLC]  = uigetfile('*.csv','Select DLC file with body parts','e:\Projects\BOF\BehaviorData\3_DLC\');
    PathOut = uigetdir('e:\Projects\BOF\BehaviorData\5_Behavior26\', 'Pick a Directory for Outputs');
    
    % loading preset file
    answer = questdlg('Do you have preset file?', 'Uploading files', 'Yes','No','Yes');
    switch answer
        case 'Yes'
            [FilenamePreset, PathPreset]  = uigetfile('*.mat','Select preset file','e:\Projects\BOF\BehaviorData\4_Presets\');
        case 'No'
...
Options.LikelihoodThreshold = 0.9;
...
Zones(end+1).name = 'middlezone';
Zones(end).type = 'area';
Zones(end).maskfilled = TempMask;

Zones(end+1).name = 'center';
Zones(end).type = 'area';
Zones(end).maskfilled = single(Zones(strcmp({Zones.name}, 'Center')).maskfilled) - single(Zones(strcmp({Zones.name}, 'middlezone')).maskfilled);

%% Task-specific Acts

if Filename(end-1) == '4'
    % for 4T
    ZonesOption.NameZone = {'ArenaCornersAllRealOut' 'ArenaWallsAllRealOut' 'Center' 'Object1RealOut' 'Object1Real' 'Object2RealOut' 'Object2Real'};
    ZonesOption.NameAct = {'corners' 'walls' 'center' 'bowl_interaction_any' 'bowl_inside' 'object1_interaction_any' 'object1_inside'};
    ZonesOption.NumBodyPart = [Point.Center Point.Center Point.Center Point.HeadCenter Point.Center Point.HeadCenter Point.Center];
elseif Filename(end-1) == '2' || Filename(end-1) == '3' || Filename(end-1) == '5'
    % for {2,3,5}T
    ZonesOption.NameZone = {'WallsAndCornersRealOut' 'middlezone' 'center' 'Object1RealOut' 'Object1Real' 'Object2RealOut' 'Object2Real'};
    ZonesOption.NameAct = {'walls' 'middle_zone' 'center' 'bowl_interaction_any' 'bowl_inside' 'object1_interaction_any' 'object1_inside'};
    ZonesOption.NumBodyPart = [Point.Center Point.Center Point.Center Point.HeadCenter Point.Center Point.HeadCenter Point.Center];
elseif Filename(end-1) == '1'
    % for 1T
...
for object = 1:size(ArenaAndObjects,2)-1
    ...
    switch object
        ...
        case 3
            DistanceObject2 = sqrt((BodyPartsTracesMainX(IndexHead,:)- CenterX).^2+(BodyPartsTracesMainY(IndexHead,:)-CenterY).^2);
            ObjectCenterY2 = CenterY;
            ObjectCenterX2 = CenterX;
    end
end
...
for object  = 1:size(ArenaAndObjects,2)-1
    switch object
        case 1
            ...
            IndexInteract = find(strcmp({Acts.ActName}, 'bowl_interaction'), 1);
        case 2
            TargetDistance = DistanceObject1;
            IndexInteract = find(strcmp({Acts.ActName}, 'object1_interaction'), 1);
        case 3
            TargetDistance = DistanceObject2;
            IndexInteract = find(strcmp({Acts.ActName}, 'object2_interaction'), 1);
    end
    ...
    %     DistanceVelocitySmoothed = smooth(DistanceVelocity, round(FramesNumEntry/8), 'moving');
    ...
    switch object
        case 1
            Acts(end+1).ActName = 'entry_in_bowl';
            Acts(end).ActArray = entryInBowl.Inside;
            Acts(end).ActArrayRefine = Acts(end).ActArray;
        case 2
            Acts(end+1).ActName = 'entry_in_object1';
            Acts(end).ActArray = entryInBowl.Inside;
            Acts(end).ActArrayRefine = Acts(end).ActArray;
        case 3
            Acts(end+1).ActName = 'entry_in_object2';
            Acts(end).ActArray = entryInBowl.Inside;
            Acts(end).ActArrayRefine = Acts(end).ActArray;
    end
...
%
% h = figure;
% %        	plot(time, DistanceVelocity, 'b'); hold on;
%         plot(time, DistanceVelocitySmoothed, 'k'); hold on;
%         plot(time, DistanceVelocitySmoothedThreshold*10, 'b'); hold on;
%         plot(time, Acts(IndexInteract).ActArrayRefine*8 , 'r'); hold on; % any bowl interaction
% %         plot(time, EntryInAll*mean(DistanceVelocitySmoothed) , 'c'); hold on; % any bowl interaction
% %         plot(time, EntryInAllV*mean(DistanceVelocitySmoothed) , 'm'); hold on; % any bowl interaction
%         plot(time, Acts(IndexInteract+1).ActArrayRefine*(mean(DistanceVelocitySmoothed)+2) , 'g'); hold on;
% %         legend('velocity','velocity_thr', 'inside','EntryInAll','EntryInAllV', 'entry');
%         legend('velocity', 'velocity_thr', 'inside', 'entry');

%% object in field of view

AngelView = 20; % in degrees
LengthViewCm = 20; % in cm
...
for object  = 1:size(ArenaAndObjects,2)-1
    switch object
        case 1
            ...
            Acts(end+1).ActName = 'bowl_in_view';
            Acts(end).ActArray = single(abs(BowlDirection-HeadDirection) < AngelView*pi/180)';
            Acts(end).ActArray = Acts(end).ActArray - Acts(9).ActArray;
            Acts(end).ActArray(Acts(end).ActArray < 0) = 0;
            Acts(end).ActArrayRefine = RefineLine(Acts(end).ActArray, Options.MinLengthActInFrames, Options.MinLengthActInFrames);
            Acts(end).ActArrayRefine = Acts(end).ActArrayRefine';
        case 2
            [ObjectDirection1,~] = cart2pol(ObjectCenterX1-CenterHead.X,ObjectCenterY1-CenterHead.Y);
            
            Acts(end+1).ActName = 'object1_in_view';
            Acts(end).ActArray = single(abs(ObjectDirection1-HeadDirection) < AngelView*pi/180)';
            Acts(end).ActArray = Acts(end).ActArray - Acts(10).ActArray;
            Acts(end).ActArray(Acts(end).ActArray < 0) = 0;
            Acts(end).ActArrayRefine = RefineLine(Acts(end).ActArray, Options.MinLengthActInFrames, Options.MinLengthActInFrames);
            Acts(end).ActArrayRefine = Acts(end).ActArrayRefine';
        case 3
            [ObjectDirection2,~] = cart2pol(ObjectCenterX2-CenterHead.X,ObjectCenterY2-CenterHead.Y);
            
            Acts(end+1).ActName = 'object2_in_view';
            Acts(end).ActArray = single(abs(ObjectDirection2-HeadDirection) < AngelView*pi/180)';
            Acts(end).ActArray = Acts(end).ActArray - Acts(11).ActArray;
            Acts(end).ActArray(Acts(end).ActArray < 0) = 0;
            Acts(end).ActArrayRefine = RefineLine(Acts(end).ActArray, Options.MinLengthActInFrames, Options.MinLengthActInFrames);
            Acts(end).ActArrayRefine = Acts(end).ActArrayRefine';
    end
...
if PlotOption.acts
    ...
    if Filename(end-1) == '1'
        act_for_plot = [8 10 11 12];
    else
        act_for_plot = [8 10 12:16];
    end
    ...
    for act = act_for_plot
        ...
        for k = videoframes(1:videoframesMax)
            ...
            % field of view
            if Acts(act).ActName == "bowl_in_view" || Acts(act).ActName == "object1_in_view" || Acts(act).ActName == "object2_in_view"
            ...
            % trajectory of mouse
            if Acts(act).ActName == "entry_in_bowl" || Acts(act).ActName == "entry_in_object1" || Acts(act).ActName == "entry_in_object2"
