% clear all;
function FreezFilt(PathFreez, FilenameFreez, filenameout)
%load line of freezing 
% [FilenameFreez, PathFreez]  = uigetfile('*.csv','load the freezing file','e:\_Projects\Olya_freez [2021]\video_pkl\');
...
% saveas(h,fullfile(PathFreez, sprintf('%s_FreezRawVsRef.fig',filenameout)));
% delete(h);
...
title(sprintf('Distribution of freezing time, %s',filenameout), 'FontSize', 18);
F = getframe(h);
...
title(sprintf('Distribution of gap between freezing, %s',filenameout), 'FontSize', 18);
F = getframe(h);
