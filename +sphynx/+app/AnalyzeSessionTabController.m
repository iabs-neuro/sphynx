    ...
    methods (Access = private)
        ...
        function refreshResults(obj)
            ...
            if isfield(r, 'BarnesMetrics') && isstruct(r.BarnesMetrics)
                bm = r.BarnesMetrics;
                % R26: header now shows all 8 fields returned by
                % barnesSessionMetrics (PrimaryErrors + FirstChecked
                % HoleNumber added).
                lines{end + 1} = sprintf( ...
                    ['Barnes: target visit #%s   |   primary errors %s   ' ...
                     '|   %d holes checked   |   first hole #%s (err %s deg)   ' ...
                     '|   mean error %s deg   |   nose-pokes %d   ' ...
                     '|   body visits %d'], ...
                    formatNum(getOr(bm, 'TargetHoleVisitOrder', NaN)), ...
                    formatNum(getOr(bm, 'PrimaryErrors', NaN)), ...
                    getOr(bm, 'NumCheckedHoles', 0), ...
                    formatNum(getOr(bm, 'FirstCheckedHoleNumber', NaN)), ...
