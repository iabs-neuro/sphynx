...
% loaded as a .mat for project-specific defaults.
    ...
    methods
        ...
        function buildTable(obj)
            ...
            % R28: pass an empty flat-Metrics list so acts WITHOUT a
            % ticked checkbox produce ZERO columns (previously the
            % flat default {ActPercent, ActDuration, ActNumber,
            % ActMeanTime} kicked in for any act that metricMatrixTo
            % Struct dropped because it had no ticks -- meaning the
            % wide table grew columns the user explicitly did not
            % want). With Metrics={}, unticked acts honour the
            % checkbox state literally.
