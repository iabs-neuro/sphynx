...
function testMouseInsideZoneActsSpatial(testCase)
    % v3 Barnes: mouse_inside_<zone> is the "fully in the hole"
    % multi-bodypart special act. Same bucket as the _at_ family.
    names = {'mouse_inside_target', 'mouse_inside_platform', ...
             'mouse_inside_hole1', 'mouse_inside_hole19'};
    for k = 1:numel(names)
        verifyEqual(testCase, sphynx.util.actBucket(names{k}), 'spatial', ...
            sprintf('expected "%s" -> spatial', names{k}));
    end
