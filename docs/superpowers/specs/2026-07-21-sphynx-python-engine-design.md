...
- **`Session`** -- dataclass: `body_parts: list[str]`, `X, Y, likelihood: np.ndarray (PxN)`,
  ...
  Метод `part(name)` резолвит по алиасам. Заменяет параллельные `BPX / BPY /
  bodyPartsNames`. Побочный эффект: синтетический центр перестаёт быть спецслучаем
  (баг R31 #1/#3 не воспроизводится по построению).
- **`Preset`** -- dataclass: `options`, `zones: list[Zone]`, `arena_and_objects`;
  `Zone = (name, type, mask, class, index, is_target, angle)`.
- **Акты** -- `Act` (name, type, category, params) + булев результат.
